...
function [u, uRef, x, t] = centeredDifference(propagatingFunction, v, t_0, t_f, M, N)
...
u(:, 1)      = propagatingFunction(x);                % --- Initial condition
uRef(:, 1)   = u(:, 1);

for l = 1 : M       % --- Time step
    u(2 : N, l + 1)     = u(2 : N, l) - (alpha / 2) * (u(3 : N + 1, l) - u(1 : N - 1, l));
