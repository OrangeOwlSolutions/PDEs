...
*
*      @param FT Field Type - Either float or double
*      @param cublasHandle must be initalized with cublasCreate
...
//template<class FT>
//int solve_with_chebyshev_iteration(cublasHandle_t const             cublasHandle,
...
//        //        preconditioner->init(M, alpha, cublasHandle, cusparse_handle); 
//        //}
//
//
//        // x = 0
...
// 
//        // r = b
//        cublas_copy(cublasHandle, n, b, r);
//        // r = r - Ax = b - Ax
//        cublas_axpy(cublasHandle, n, &FT_MINUS_ONE, Ax, r);
//
//        FT norm0;
//        cublas_nrm2(cublasHandle, n, r, &norm0);
//
//        int num_iter;
...
//                if (num_iter == 1) {
//                        // p = z
//                        cublas_copy(cublasHandle, n, z, p);
//
//                        fa = FT(1) / d;
//                } else {
//                        fb = (c*fa/FT(2));
...
//
//                        // p = fb * p
//                        cublas_scal(cublasHandle, n, &fb, p);
//
//                        // p = p + z
//                        cublas_axpy(cublasHandle, n, &FT_ONE, z, p);
//                }
//
//                // x = x + fa*p
//                cublas_axpy(cublasHandle, n, &fa, p, x);
//
//                // Ax = A*x
//                multiply_by_A<FT><<<iDivUp(n, 1024), 1024>>>(M, alpha, x, Ax);
//           
...
//                
//                FT norm;
//                cublas_nrm2(cublasHandle, n, r, &norm);
...
*/
template<class FT>
int solve_with_conjugate_gradient3D(cublasHandle_t   const cublasHandle,
	cusparseHandle_t const cusparse_handle,
	int const              M,
	FT const               alpha,
	FT const * const       b,
	...
	//Preconditioner<FT> *   preconditioner = NULL ) 
{
	...
	//if (preconditioner == NULL) {
	// p = r
	//        cublas_copy(cublasHandle, n, r, p);
	//} else {
	//        // p = M \ r
	//        preconditioner->run(r, p);
	//        // q = p
	//        cublas_copy(cublasHandle, n, p, q);
	//}
	int iter_num;
	for (iter_num = 1; iter_num <= maxIter; ++iter_num) {
		//if (preconditioner == NULL) {
		// beta = <r, r>
		//        cublas_dot(cublasHandle, n, r, r, &beta);
		//} else {
		//        // beta = <r, q>
		//        cublas_dot(cublasHandle, n, r, q, &beta);
		//}
		...
		// ph = <p, h>
		cublas_dot(cublasHandle, n, p, h, &ph);

		c = beta / ph;

		//  x = x + c*p
		cublas_axpy(cublasHandle, n, &c, p, x);

		// r = r - c*h
		FT minus_c = -c;
		cublas_axpy(cublasHandle, n, &minus_c, h, r);
		...
		// rr = <r, r>
		FT rr;
		cublas_dot(cublasHandle, n, r, r, &rr);

		beta = rr / beta;
		//} else {
		//        // rq = <r, q>
		//        FT rq;
		//        cublas_dot(cublasHandle, n, r, q, &rq);

		//        beta = rq / beta;
		//}

		// p = beta * p
		cublas_scal(cublasHandle, n, &beta, p);

		//if (preconditioner == NULL) {
		// p = r + p
		cublas_axpy(cublasHandle, n, &FT_ONE, r, p);
		//} else {
		//        // p = q + p
		//        cublas_axpy(cublasHandle, n, &FT_ONE, q, p);
		//}
	}
