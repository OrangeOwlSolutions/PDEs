...
/**************************************************/
/* ATOMICADD FOR DOUBLES AND FOR PRE-PASCAL CARDS */
/**************************************************/
...
int main(void) {
	...
	// --- Host and device residuals for both red and black temperature points
	real *h_L2norm_pointwise = (real *)malloc(sizeof(real));
	real *d_L2norm_pointwise; gpuErrchk(cudaMalloc(&d_L2norm_pointwise, sizeof(real)));
	gpuErrchk(cudaMemset(d_L2norm_pointwise, 0, sizeof(real)));
	...
	for (iter = 1; iter <= maxIter; ++iter) {
		...
		// --- Copy squared L2 norm (first part for red cells)
		gpuErrchk(cudaMemcpy(h_L2norm_pointwise, d_L2norm_pointwise, sizeof(real), cudaMemcpyDeviceToHost));
		L2norm = h_L2norm_pointwise[0];

		// --- Update black cells
		blackKernel << <dimGrid, dimBlock >> > (d_Tred, d_Tblack, d_L2norm_pointwise, thermalConductivity, dx, dy, width, TN, NUM, NUM);

		// --- Copy squared L2 norm (second part for red cells)
		gpuErrchk(cudaMemcpy(h_L2norm_pointwise, d_L2norm_pointwise, sizeof(real), cudaMemcpyDeviceToHost));
		L2norm = L2norm + h_L2norm_pointwise[0];
