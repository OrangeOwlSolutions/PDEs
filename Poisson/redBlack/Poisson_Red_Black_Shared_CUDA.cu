...
__global__ void redKernel(const real * d_Tblack, real * d_Tred, real * L2norm, real thermalConductivity, real dx, real dy, real width, real TN, const int rowmax, const int colmax) {
	...
	if (threadIdx.x == 0) L2norm[blockIdx.y + (gridDim.y * blockIdx.x)] = res_cache[0];
}
...
int main(void) {
	...
	real *d_L2norm_pointwise; gpuErrchk(cudaMalloc(&d_L2norm_pointwise, dimGrid.x * dimGrid.y * sizeof(real)));
	gpuErrchk(cudaMemset(d_L2norm_pointwise, 0, dimGrid.x * dimGrid.y * sizeof(real)));
	...
	for (iter = 1; iter <= maxIter; ++iter) {
		...
		L2norm = thrust::reduce(thrust::device_pointer_cast(d_L2norm_pointwise), thrust::device_pointer_cast(d_L2norm_pointwise) + dimGrid.x * dimGrid.y);
		...
		L2norm = L2norm + thrust::reduce(thrust::device_pointer_cast(d_L2norm_pointwise), thrust::device_pointer_cast(d_L2norm_pointwise) + dimGrid.x * dimGrid.y);
