...
#define REALTYPE		0			// --- 0 for float and 1 for double
...
template<class T>
__global__ void updateKernelNoSharedNotWorking(T * __restrict__ d_u1, T * __restrict__ d_u2, T * __restrict__ d_u3, T * __restrict__ d_u, const T * __restrict__ d_t, const T * __restrict__ d_x, const T v, const T x1, 
...
template<class T>
__global__ void updateKernelNoShared(T * __restrict__ d_u1, T * __restrict__ d_u2, T * __restrict__ d_u3, T * __restrict__ d_u, const T * __restrict__ d_t, const T * __restrict__ d_x, const T v, const T x1,
	const T x2, const T alpha, const int l, const int N) {
...
int main() {
	...
	const REAL t_0 = 0.f;								// --- Initial time
	const REAL t_f = 15.f;								// --- Final time
	const REAL x_0 = 0.f;								// --- Left boundary of the computational domain
	const REAL x_f = 2.f * PI;							// --- Right boundary of the computational domain
	...
	/************************/
	const REAL	dx = 2.f * PI / (REAL)N;				// --- Discretization step in space
	...
	saveGPUrealtxt(d_u, "D:\\Project\\FDTD\\FDTD_1D_Acoustics\\FDTD_1D_Acoustics_CUDA\\FDTD_1D_Acoustics_CUDA\\d_u.txt", (M + 1) * (N + 1));
