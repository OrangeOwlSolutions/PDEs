...
/***********************************/
void updateHost(const double * __restrict__ h_uold, const double * __restrict__ h_u, double * __restrict__ h_unew, const double c, const double q, const double r, const int Nx, const int Ny) {
	
	for (int j = 1; j < Ny - 1; j++)
		for (int i = 1; i < Nx - 1; i++)
			h_unew[j * Nx + i] = q * h_u[j * Nx + i] + r * h_uold[j * Nx + i] + c * (h_u[j * Nx + i - 1] + h_u[j * Nx + i + 1] + h_u[(j + 1) * Nx + i] + h_u[(j - 1) * Nx + i] - 4. * h_u[j * Nx + i]);
...
__global__ void updateDevice_v0(const double * __restrict__ d_uold, const double * __restrict__ d_u, double * __restrict__ d_unew, const double c,
								const double q, const double r, const int Nx, const int Ny) {
	...
	d_unew[tidy * Nx + tidx] = q * d_u[tidy * Nx + tidx] + r * d_uold[tidy * Nx + tidx] + c * (d_u[tidy * Nx + tidx - 1] + 
																							   d_u[tidy * Nx + tidx + 1] + 
																							   d_u[(tidy + 1) * Nx + tidx] + 
																							   d_u[(tidy - 1) * Nx + tidx] - 
																							   4. * d_u[tidy * Nx + tidx]);
...
__global__ void updateDevice_v1(const double * __restrict__ d_uold, const double * __restrict__ d_u, double * __restrict__ d_unew, const double c,
	...
	if ((threadIdx.x > 0) && (threadIdx.x < (BLOCKSIZEX - 1)) && (threadIdx.y > 0) && (threadIdx.y < (BLOCKSIZEY ‐ 1)) && (tidx < Nx - 1) && (tidy < Ny - 1)) 
		...
		d_unew[tidy * Nx + tidx] = q * d_u_sh[threadIdx.x][threadIdx.y] + 
			                       r * d_uold[tidy * Nx + tidx] + 
								   c * (d_u_sh[threadIdx.x - 1][threadIdx.y] +
...
/********************************************************/
__global__ void updateDevice_v2(const double * __restrict__ d_uold, const double * __restrict__ d_u, double * __restrict__ d_unew, const double c,
	const double q, const double r, const int Nx, const int Ny) {
...
/********************************************************/
__global__ void updateDevice_v3(const double * __restrict__ d_uold, const double * __restrict__ d_u, double * __restrict__ d_unew, const double c,
	const double q, const double r, const int Nx, const int Ny) {
	...
	d_unew[tidy * Nx + tidx] = q * d_u_sh[threadIdx.x + 1][threadIdx.y + 1] +
									r * d_uold[tidy * Nx + tidx] +
									c * (d_u_sh[threadIdx.x][threadIdx.y + 1] +
...
int main()
{
	...
	const double	p = 0.02;															// --- Wave decay factor
	...
	/* ITERATIONS - HOST */
	/*********************/
	const double	c = dt * dt * v * v / (dx * dx);									// --- CFL number
	const double	q = 2. - p * dt;
	const double	r = -1. + p * dt;

	for (int tt = 0; tt < T; tt++) {
		updateHost(h_uold, h_u, h_unew, c, q, r, Nx, Ny);
	...
	for (int tt = 0; tt < T; tt++) {
		//updateDevice_v0<<<Grid, Block>>>(d_uold, d_u, d_unew, c, q, r, Nx, Ny);
		//updateDevice_v1<<<Grid, Block>>>(d_uold, d_u, d_unew, c, q, r, Nx, Ny);
		//updateDevice_v2 << <Grid, Block >> >(d_uold, d_u, d_unew, c, q, r, Nx, Ny);
		updateDevice_v3 << <Grid, Block >> >(d_uold, d_u, d_unew, c, q, r, Nx, Ny);
#ifdef DEBUG 
	...
	saveCPUrealtxt(h_u, "D:\\Project\\FDTD\\FDTD_2D_Acoustics\\FDTD2D_hostResult.txt", Nx * Ny);
	...
	saveCPUrealtxt(h_uDevice, "D:\\Project\\FDTD\\FDTD_2D_Acoustics\\FDTD2D_deviceResult.txt", Nx * Ny); 
